...
export class UserService {
  ...
  async createUser(userData: BaseUser): Promise<User> {
    try {
      ...
      const newUser = await userRepository.create(validatedData as BaseUser)
  ...
  async seedFromJsonPlaceholder(): Promise<{ message: string; count: number }> {
    try {
      ...
      const createdUsers = await userRepository.createMany(usersToInsert)
      const createdCount = createdUsers.length
      const createdWord = pluralizeUser(createdCount)
      logger.info(`Database seeding completed successfully`, { createdCount: createdUsers.length })
  ...
  async clearAllUsers(): Promise<{ message: string; count: number }> {
    try {
      ...
      const deletedCount = await userRepository.deleteAll()
